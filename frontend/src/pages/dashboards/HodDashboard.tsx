...
export const HodDashboard = () => {
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEventMsg(
        cancelledCount > 0
          ? `✅ Event created. ${cancelledCount} leave(s) automatically cancelled.`
          : '✅ Event created successfully.'
      );
      await fetchData();
      setTimeout(() => { setShowEventModal(false); setEventMsg(''); setEventForm({ title: '', date: '', type: 'EXAM' }); }, 2500);
    } catch (err: any) {
      setEventMsg(err.response?.data?.message || 'Failed to create event.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="font-bold text-gray-800">Pending Leave Requests (Students & Professors)</h3>
        </div>
        {leaves.length === 0 ? (
          ...
          <table className="min-w-full divide-y divide-gray-100">
            <thead className="bg-gray-50">
              <tr>
                {['Applicant', 'Role', 'Type', 'Dates', 'Reason', 'Actions'].map((h) => (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-6">
              ...
              {events.map((ev) => (
                <div key={ev.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50">
                  <div className="flex items-center gap-3">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${EVENT_BADGE[ev.type] ?? 'bg-gray-100 text-gray-600'}`}>
                      {ev.type}
                    </span>
          ...
          <p className="mt-4 text-xs text-gray-400 flex items-center gap-1">
            <AlertTriangle size={11} /> Adding an EXAM or RESTRICTED event will automatically cancel all overlapping leaves.
          </p>
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="mb-4 bg-red-50 border border-red-100 text-red-700 rounded-lg px-3 py-2 text-xs font-medium">
                ⚠ Adding an EXAM or RESTRICTED event will <strong>automatically cancel</strong> all pending and approved leaves on that date.
              </div>
              ...
              <form onSubmit={handleCreateEvent} className="space-y-4">
                ...
                {eventMsg && (
                  <p className={`text-sm font-medium ${eventMsg.startsWith('✅') ? 'text-green-600' : 'text-red-600'}`}>
                    {eventMsg}
                  </p>
                )}
      ...
      {showLeaveModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Pending student/professor requests will auto-forward to Principal during your absence.</p>
            <form onSubmit={handleMarkLeave} className="space-y-4">
              ...
              <button type="submit" disabled={leaveSubmitting}
                className={`w-full py-2 rounded-lg text-sm font-semibold text-white ${leaveSubmitting ? 'bg-gray-400' : 'bg-amber-500 hover:bg-amber-600'}`}>
                {leaveSubmitting ? 'Marking...' : 'Confirm Leave'}
              </button>
            </form>
          </div>
        </div>
